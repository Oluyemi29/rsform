"use client";
import { DeleteMembers } from "@/app/api/Controller";
...
const MemberInfo = ({ allMembers }: MembersInfoProps) => {
  ...
  const handleDelete = async (
    ...
    try {
      ...
      const response = await DeleteMembers(id, image);
      if (response.success) {
        addToast({
          title: "Done",
  ...
  return (
    <div className="w-full flex flex-row justify-center bg-white pt-10">
      <Card className="lg:w-2/6 md:w-4/6 p-5 w-full h-[28rem] overflow-y-auto no-scrollbar flex flex-col gap-5">
        <p className="text-center text-rsdeep">Members Info</p>
        <div className="w-max mx-auto flex mt-5 flex-col justify-center items-center">
          <Image
            src={allMembers?.image || "/user.jpg"}
            alt="user"
            width={40}
            height={40}
            priority
        ...
        <div className="flex flex-row justify-center items-center">
          {loading ? (
            ...
            <Button
              onPress={() => handleDelete(allMembers?._id, allMembers?.image)}
